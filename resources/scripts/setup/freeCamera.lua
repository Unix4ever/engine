return function(self)
  ...
  local handleMouseEvent = function(event)
    ...
    elseif event.type == MouseEvent.MOUSE_MOVE and rotate then
      yawNode:yaw(Degree.new(-event.relX * 0.005))
      pitchNode:pitch(Degree.new(-event.relY * 0.005))
    end
  end
  ...
  print(core.settings.render.camera.type:str())

  rollNode:getCamera("camera"):attach(core:render().viewport)
end
