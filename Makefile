...
.deps: .repo conanfile.py
	conan install -g cmake -s build_type=$(CMAKE_BUILD_TYPE) -o gsage:with_ogre=$(OGRE_VERSION) -o gsage:with_librocket=$(WITH_LIBROCKET) -o with_lua_version=$(LUA_VERSION)  --build=outdated .
	@touch .deps
