...
struct DockContext
{
    ...
    ImVec2 m_previous_workspace_size;
    ...
    if(m_previous_workspace_size.x != m_workspace_size.x || m_previous_workspace_size.y != m_workspace_size.y) {
      for (int i = 0; i < m_docks.size(); ++i) {
        Dock& dock = *m_docks[i];
  ...
  bool drawTabbarListButton(Dock& dock)
  {
    if (!dock.next_tab) return false;

    ImDrawList* draw_list = GetWindowDrawList();
    if (InvisibleButton("list", ImVec2(16, 24)))
    {
    ...
    float xPosition = center.x + 4;
    float yPosition = min.y + 6;
    draw_list->AddRectFilled(ImVec2(xPosition - 4, yPosition + 3),
        ImVec2(xPosition + 4, yPosition + 5),
        hovered ? color_active : text_color);
    draw_list->AddTriangleFilled(ImVec2(xPosition - 4, yPosition + 7),
        ImVec2(xPosition + 4, yPosition + 7),
        ImVec2(xPosition, yPosition + 12),
        hovered ? color_active : text_color);
    return true;
  }
  ...
  {
    PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(TABBAR_PADDING_X, TABBAR_PADDING_Y));
    PushStyleVar(ImGuiStyleVar_ChildWindowRounding, 0.0f);
    float tabbar_height = GetTextLineHeightWithSpacing() + TABBAR_PADDING_Y;
    ...
    PushStyleColor(ImGuiCol_ChildWindowBg, GetColorU32(ImGuiCol_Border));
    if (BeginChild(tmp, size, false, ImGuiWindowFlags_NoScrollbar))
    {
      PopStyleColor();
      Dummy(ImVec2(0, 0));
      Dock* dock_tab = &dock;
      ...
      ImU32 color_active = GetColorU32(ImGuiCol_PopupBg);
      ImU32 color_hovered = GetColorU32(ImGuiCol_PopupBg);
      ImU32 text_color = GetColorU32(ImGuiCol_Text);
      float line_height = GetTextLineHeightWithSpacing();
      float tab_base;
      bool first = true;
      bool sameLine = drawTabbarListButton(dock);

      Dock* selection = nullptr;
      ...
      while (dock_tab)
      {
        ...
        ImVec2 windowPadding = GetStyle().WindowPadding;
        size += windowPadding;
        ...
        if (dock_tab->active && close_button) {
          size.x += size.y + GetStyle().ItemSpacing.x;
        }
        tab_base = pos.y;
        float leftCurveRadius = first ? 5.0f : 0.0f;
        float rightCurveRadius = dock_tab->next_tab ? 0.0f : 5.0f;
        float paddingX = 7;
        draw_list->PathClear();
        draw_list->PathLineTo(pos + ImVec2(-paddingX, size.y));
        draw_list->PathLineTo(pos + ImVec2(-paddingX, leftCurveRadius));
        if(leftCurveRadius > 0) {
          draw_list->PathBezierCurveTo(pos + ImVec2(-paddingX, leftCurveRadius),
              pos + ImVec2(-paddingX, 0),
              pos + ImVec2(-paddingX+leftCurveRadius, 0),
              10);
        }
        draw_list->PathLineTo(pos + ImVec2(size.x + paddingX - rightCurveRadius, 0));
        if(rightCurveRadius > 0) {
          draw_list->PathBezierCurveTo(pos + ImVec2(size.x + paddingX - rightCurveRadius, 0),
              pos + ImVec2(size.x + paddingX, 0),
              pos + ImVec2(size.x + paddingX, rightCurveRadius),
              10);
        }
        draw_list->PathLineTo(pos + ImVec2(size.x + paddingX, size.y));
        ...
        {
          SameLine();
          tab_closed = InvisibleButton("close", ImVec2(size.y, size.y));
  ...
  bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags)
  {
    ...
    pos.y += tabbar_height + TABBAR_PADDING_Y;
    size.y -= tabbar_height + TABBAR_PADDING_Y;
    ...
    ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
      ...
      extra_flags;
    //PushStyleColor(ImGuiCol_ChildWindowBg, ImVec4(1.0, 1.0, 1.0, 1.0));
  ...
  Gsage::DataProxy getState()
  {
    Gsage::DataProxy state = Gsage::DataProxy::create(Gsage::DataWrapper::JSON_OBJECT);
    for (int i = 0; i < m_docks.size(); ++i)
    {
      Dock& dock = *m_docks[i];
      fillLocation(dock);
      Gsage::DataProxy dockData;
      dockData.put("label", std::string(dock.label));
      dockData.put("x", dock.pos.x);
      dockData.put("y", dock.pos.y);
      dockData.put("location", std::string(dock.location));
      dockData.put("sizeX", dock.size.x);
      dockData.put("sizeY", dock.size.y);
      dockData.put("status", (int)dock.status);
      dockData.put("opened", (int)dock.opened);
      dockData.put("active", (int)dock.active);
      dockData.put("prev", getDockIndex(dock.prev_tab));
      dockData.put("next", getDockIndex(dock.next_tab));
      dockData.put("child0", getDockIndex(dock.children[0]));
			dockData.put("child1", getDockIndex(dock.children[1]));
      dockData.put("parent", getDockIndex(dock.parent));
      state.push(dockData);
    }
    return state;
  }
  ...
  void setState(const Gsage::DataProxy& docks)
  {
    m_previous_workspace_size = m_workspace_size;
    ...
    for(auto pair : docks)
    {
      Gsage::DataProxy& data = pair.second;
      Dock& dock = *m_docks[index++];
      dock.last_frame = 0;
      dock.invalid_frames = 0;
      dock.label = ImStrdup(data.get<std::string>("label", "").c_str());
      dock.id = ImHash(dock.label, 0);
      dock.pos.x = data.get<float>("x", 0);
      dock.pos.y = data.get<float>("y", 0);
      dock.size.x = data.get<float>("sizeX", 0);
      dock.size.y = data.get<float>("sizeY", 0);
      dock.active = data.get<int>("active", 0);
      dock.opened = data.get<int>("opened", 0);
      std::string loc = data.get<std::string>("location", "");
      strncpy(dock.location, loc.c_str(), loc.size());
      dock.status = (Status_)data.get<int>("status", Status_Docked);
      dock.prev_tab = getDockByIndex(data.get<int>("prev", -1));
      dock.next_tab = getDockByIndex(data.get<int>("next", -1));
      dock.children[0] = getDockByIndex(data.get<int>("child0", -1));
      dock.children[1] = getDockByIndex(data.get<int>("child1", -1));
      dock.parent = getDockByIndex(data.get<int>("parent", -1));
      if(dock.children[0] || dock.children[1]) {
        dock.setChildrenPosSize(ImVec2(dock.pos.x, dock.pos.y), ImVec2(dock.size.x, dock.size.y));
      }
    }
  }
};
...
Gsage::DataProxy getDockState()
{
  return g_dock.getState();
}

void setDockState(const Gsage::DataProxy& state)
{
  return g_dock.setState(state);
}
